...
def evaluate(hps):
    # 构建输入数据(读取队列执行器）
    images, labels = cifar_input.get_test_batch_data(100)
    ...
    while True:
        # 检查checkpoint文件
        try:
            ckpt_state = tf.train.get_checkpoint_state("/home/ocean1100/tensorflow/TensorResnet/save/")
        except tf.errors.OutOfRangeError as e:
            tf.logging.error('Cannot restore checkpoint: %s', e)
            continue
        if not (ckpt_state and ckpt_state.model_checkpoint_path):
            tf.logging.info('No model to eval yet at %s', "/home/ocean1100/tensorflow/TensorResnet/save/")
            continue
        ...
        for _ in range(100):
            ...
            train_correct_pred = tf.equal(tf.argmax(predictions, 1), tf.argmax(train_labels, 1))
